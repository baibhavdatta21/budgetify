...
WITH monthly_totals AS (
    SELECT 
        ...
        ROUND(SUM(amount) / COUNT(DISTINCT day), 2) AS avg_daily_expense  -- Assuming 'day' column for daily average
    FROM wrk
    GROUP BY month
),
...
expense_growth AS (
    SELECT 
        month,
        total_expense,
        LAG(total_expense) OVER (ORDER BY month) AS previous_month_expense,
        ROUND((total_expense - LAG(total_expense) OVER (ORDER BY month)) / 
              NULLIF(LAG(total_expense) OVER (ORDER BY month), 0) * 100, 2) AS growth_rate
    FROM monthly_totals
)
...
SELECT 
    ...
    mt.total_expense,
    mt.total_investment,
    mt.avg_daily_expense,
    ce.category,
    ce.category_expense,
    nw.total_needs,
    nw.total_wants,
    eg.previous_month_expense,
    eg.growth_rate
FROM monthly_totals AS mt
LEFT JOIN category_expenses AS ce ON mt.month = ce.month
LEFT JOIN needs_vs_wants AS nw ON mt.month = nw.month
LEFT JOIN expense_growth AS eg ON mt.month = eg.month
ORDER BY mt.month, ce.category;
